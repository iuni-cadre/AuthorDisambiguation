...
def convert(
    ...
    _insert_tmpl = "INSERT INTO %s VALUES (%s)" % (
        table,
        ",".join(["?"] * len(headers)),
    )
...
def _guess_types(reader, number_of_columns, max_sample_size=100):
    ...
    perresult = {"integer": 0, "real": 0, "text": 0}
...
"""
    ...
    group.add_argument(
        "--bz2", help="Input csv file is compressed using bzip2.", action="store_true"
    )
    group.add_argument(
        "--gzip", help="Input csv file is compressed using gzip.", action="store_true"
    )
