...
def get_paper_address(paper_ids, conn):
    ...
    # But doing this may require more memory because the join clause will be pefermed after where clause
    return pd.read_sql(
        "select * from address_table where paper_id in ({paper_ids}) ".format(
            paper_ids=to_string_array(paper_ids),
        ),
        conn,
    )
...
# Scoring functions
#
def matching_score_by_authors(author_paper_table, general_name_list):
    """
...
def matching_score_by_articles(author_paper_table, block_id, conn):
    ...
    name_ids = get_name_ids_in_block(block_id, conn)
    ...
    paper_author_mat, pids, _ = to_binary_matrix(df["index"], df["short_name_id"])
    ...
    W = (name_paper_by_paper @ W @ name_paper_by_paper.T).toarray()
    return W 
...
def matching_score_by_citations(author_paper_table, conn):
    ...
    U, _, _ = to_binary_matrix(
        cited_paper_table["index"].values, cited_paper_table["source"].values, num_rows=num_papers
    )
    ...
    U, _, _ = to_binary_matrix(
        citing_paper_table["index"].values, citing_paper_table["target"].values, num_rows=num_papers
    )
    ...
    # Rules 9 
    #
    citing_paper_table = citing_paper_table[np.isin(citing_paper_table["target"].values, paper_ids)]
    node_by_paper,_,_ = to_binary_matrix(citing_paper_table["index"].values, citing_paper_table["target"].values, num_rows = author_paper_table.shape[0])
