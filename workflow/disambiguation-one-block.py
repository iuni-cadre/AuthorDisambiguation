...
if __name__ == "__main__":
    ...
    tables = {}
    for table_name in ["block_table", "citing_table","grant_table","name_paper_address","paper_address","paper_table", "name_table", "name_paper_table"]:
        if "table" not in table_name:
            table_name_extended = table_name + "_table"
        else:
            table_name_extended = table_name
        tables[table_name_extended] = pd.concat([ pd.read_csv(f, escapechar="\\") for f in glob.glob(str(root / table_name / block_name / "*"))])
